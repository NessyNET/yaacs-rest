...
public class AnnJooxParser {
    ...
    public List<AnnAnime> extractAnime(final String xml) {
        ...
        $(xml).find("anime").forEach(animeDoc -> {
            ...
            annAnime.getAltTitles()
                    .addAll(parseAltTitles($(animeDoc).find("info").filter(attr("type", "Alternative title"))));
            annAnime.getGeneres().addAll(parseGenres($(animeDoc).find("info").filter(attr("type", "Genres"))));
            annAnime.getGeneres().addAll(parseGenres($(animeDoc).find("info").filter(attr("type", "Themes"))));
            annAnime.setSummary(parseSummary($(animeDoc).find("info").filter(attr("type", "Plot Summary"))));
            annAnime.setEpisodes(
                    parseEpisodeCount($(animeDoc).find("info").filter(attr("type", "Number of episodes"))));

            String[] vintageDates = $(animeDoc).find("info").filter(attr("type", "Vintage")).text().split(" to ");
            annAnime.setStartDate(parseStartDate(vintageDates));
            annAnime.setEndDate(parseEndDate(vintageDates));
            annAnime.getWebsites()
                    .addAll(parseWebsites($(animeDoc).find("info").filter(attr("type", "Official website"))));
    ...
    protected List<AnnAnimeAltTitle> parseAltTitles(final Match altTitleMatch) {
        ...
        $(altTitleMatch).forEach(altTitle -> {
            ...
            annAnimeAltTitle.setGid(Long.parseLong($(altTitle).attr("gid")));
